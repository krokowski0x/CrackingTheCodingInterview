export class Listy {
  ...
  elementAt(i) {
    if (i >= this.container.length || i < 0) return -1;
    else return this.container[i];
  }
}
...
const findEnd = (listy, last, term) => {
  if (last === undefined && term === undefined) {
    last = 0;
    term = 1;
    if (listy.elementAt(last) === -1) return 0;
  }
  if (listy.elementAt(last) !== -1 && listy.elementAt(term) !== -1)
    ...
    if (last === term - 1) return term;

    const half = Math.floor((term - last) / 2);
    const mid = last + half;

    if (listy.elementAt(mid) === -1) return findEnd(listy, last, mid);
    else return findEnd(listy, mid, term);
  } else
...
export function sortedSearchNoSize(listy, value, front, back) {
  if (!listy) return "what are we searching?";
